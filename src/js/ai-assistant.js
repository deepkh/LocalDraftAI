(function () {
  ...
  function createAiAssistant(context) {
    ...
    function setReviewBusy(action, selection) {
      ...
      var providerLabel = settings.providerLabel || (mode === "server" ? "OpenAI-compatible custom" : "Local mock");
      ...
      if (mode === "server") {
        ...
        if (settings.reasoning && settings.reasoning.enabled) {
          appendReviewLog("Reasoning mode: " + (ME.aiReasoning ? ME.aiReasoning.label(settings.reasoning) : settings.reasoning.effort) + ".");
          if (settings.reasoning.showSummary) {
    ...
    async function requestAction(actionId, options) {
      ...
      try {
        ...
        if (provider && typeof provider.runDetailed === "function") {
          providerResult = await provider.runDetailed(actionId, range.text);
